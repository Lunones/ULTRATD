...
public class UserController extends Controller implements Initializable {
    ...
    public void onOpen(Object input) throws IOException {
        ...
        if (us == null) {
            ...
            txtID.setText(UserDAO.build().ultid() + "");
            txtLVL.setText(2+"");
            l=Integer.parseInt(txtLVL.getText());


        }else if(us.getLvl()==0 && us.getLvl()!=99){
        ...
        else if(us.getLvl()==99 || us.getLvl()>0) {
            String namefact = FactionDAO.build().findById(us.getId_faction()).getName();
            txtID.setText(UserDAO.build().ultid() + "");
            txtID.setEditable(false);
    ...
    void Backbt(ActionEvent event) throws IOException {
        if (us == null || l != 1){
        App.currentController.changeScene(Scenes.SUCHOOSE, null);  // Change scene to SU_CHOOSE
            } else if (l == 1){
